...
async function main() {
  ...
  rows.forEach(([code, country]) => {
    ...
    normalizeArray(country.military?.conflicts).forEach(conflict => {
      ...
      if (!hasValue(conflict?.startYear) && !hasValue(conflict?.endYear)) {
        warnings.push({ code, type: "military.conflicts.startYear", detail: `conflicto sin fecha (${conflict?.name || "s/n"})` });
      }
    });
  });
