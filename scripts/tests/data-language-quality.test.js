...
const prioritySafeBatchExpected = [
  {
    name: "Batalla de las Islas Paracelso (1974)",
    parent: "Disputa territorial del mar de China Meridional",
    codes: ["CHN", "VNM"]
  },
  {
    name: "Batalla de St\u00e4ket (1719)",
    parent: "Gran Guerra del Norte",
    codes: ["SWE", "RUS"]
  },
  {
    name: "Batalla por el Castillo Itter (1945)",
    parent: "Segunda Guerra Mundial",
    codes: ["USA", "DEU", "AUT", "FRA"]
  }
];
for (const expected of prioritySafeBatchExpected) {
  for (const code of expected.codes) {
    const entries = countries[code]?.military?.conflicts?.filter(item => item.name === expected.name) || [];
    assert.equal(entries.length, 1, expected.name + " debe aparecer una sola vez en " + code);
    assert.equal(entries[0].parent, expected.parent, expected.name + " debe conservar padre curado en " + code);
...
const campecheAntivariExpected = [
  {
    name: "Batalla naval de Campeche (1843)",
    parent: "Conflicto entre el gobierno central de M\u00e9xico y Yucat\u00e1n",
    codes: ["MEX", "USA"]
  },
  {
    name: "Batalla de Antivari (1914)",
    parent: "Primera Guerra Mundial",
    codes: ["FRA", "GBR", "AUT", "HUN", "MNE"]
  }
];
for (const expected of campecheAntivariExpected) {
  for (const code of expected.codes) {
    const entries = countries[code]?.military?.conflicts?.filter(item => item.name === expected.name) || [];
    assert.equal(entries.length, 1, expected.name + " debe aparecer una sola vez en " + code);
    assert.equal(entries[0].parent, expected.parent, expected.name + " debe conservar padre curado en " + code);
...
const flintDoggerExpected = [
  {
    name: "Batalla de Flint Creek (1789)",
    parent: "Guerras cheroqui-estadounidenses (1776-1794)",
    codes: ["USA"]
  },
  {
    name: "Batalla de Dogger Bank (1781)",
    parent: "Cuarta guerra anglo-neerlandesa (1780-1784)",
    codes: ["GBR", "NLD"],
    excludedCodes: ["DNK"]
  }
];
for (const expected of flintDoggerExpected) {
  for (const code of expected.codes) {
    const entries = countries[code]?.military?.conflicts?.filter(item => item.name === expected.name) || [];
    assert.equal(entries.length, 1, expected.name + " debe aparecer una sola vez en " + code);
    assert.equal(entries[0].parent, expected.parent, expected.name + " debe conservar padre curado en " + code);
  ...
  for (const code of expected.excludedCodes || []) {
    const entries = countries[code]?.military?.conflicts?.filter(item => item.name === expected.name) || [];
    assert.equal(entries.length, 0, expected.name + " no debe conservar una asociacion de pais incorrecta en " + code);
  }
  const detailIndex = (conflictDetailsIndex.conflicts || []).find(entry => entry.name === expected.name);
  assert.ok(detailIndex?.path, expected.name + " debe conservar una ficha diferida indexada");
  const detail = JSON.parse(fs.readFileSync(detailIndex.path, "utf8"));
...
const dasmanRachadoExpected = [
  {
    name: "Batalla del palacio Dasman (1990)",
    parent: "Invasi\u00f3n iraqu\u00ed de Kuwait (1990)",
    codes: ["IRQ", "KWT"],
    excludedCodes: ["USA"]
  },
  {
    name: "Batalla de Cabo Rachado (1606)",
    parent: "Guerra luso-neerlandesa (1602-1663)",
    codes: ["PRT", "NLD"]
  }
];
for (const expected of dasmanRachadoExpected) {
  for (const code of expected.codes) {
    const entries = countries[code]?.military?.conflicts?.filter(item => item.name === expected.name) || [];
    assert.equal(entries.length, 1, expected.name + " debe aparecer una sola vez en " + code);
    assert.equal(entries[0].parent, expected.parent, expected.name + " debe conservar padre curado en " + code);
...
const beitangTeacapanExpected = [
  {
    name: "Batalla de Beitang (1900)",
    parent: "Rebeli\u00f3n de los Boxers",
    codes: ["CHN", "RUS", "DEU", "FRA"],
    excludedCodes: ["USA"]
  },
  {
    name: "Batalla de Boca Teacapan (1870)",
    parent: "Pirater\u00eda en Am\u00e9rica del Norte",
    codes: ["USA", "MEX"]
  }
];
for (const expected of beitangTeacapanExpected) {
  for (const code of expected.codes) {
    const entries = countries[code]?.military?.conflicts?.filter(item => item.name === expected.name) || [];
    assert.equal(entries.length, 1, expected.name + " debe aparecer una sola vez en " + code);
    assert.equal(entries[0].parent, expected.parent, expected.name + " debe conservar padre curado en " + code);
...
const oselVaileleExpected = [
  {
    name: "Batalla de \u00d6sel (1719)",
    parent: "Gran Guerra del Norte",
    codes: ["SWE", "RUS", "EST"]
  },
  {
    name: "Primera batalla de Vailele (1888)",
    parent: "Primera guerra civil de Samoa",
    codes: ["DEU"],
    excludedCodes: ["USA"]
  }
];
for (const expected of oselVaileleExpected) {
  for (const code of expected.codes) {
    const entries = countries[code]?.military?.conflicts?.filter(item => item.name === expected.name) || [];
    assert.equal(entries.length, 1, expected.name + " debe aparecer una sola vez en " + code);
    assert.equal(entries[0].parent, expected.parent, expected.name + " debe conservar padre curado en " + code);
