...
- CI confirmo la recuperacion de la captura del loader ausente y el mismo error antes del runtime principal; ambos recorridos usan la captura acotada. La suite completa dispone de 20 minutos: el runner consumio 13 minutos antes de terminar arrastres, paneles, recorridos finales, offline y visuales. No se amplian los limites individuales de arranque, FPS, long tasks ni tamanos.
...
- La E2E del scheduler comprueba desplazamiento real de la camara y contacto activo despues del gesto, no un `moveStart` transitorio que puede haber terminado antes de consultar. Conserva el plazo de 3 segundos y las aserciones sobre tareas diferidas. `--journeys-only` permite diagnosticar los recorridos finales sin repetir el resto; `npm test` sigue ejecutando todos.
