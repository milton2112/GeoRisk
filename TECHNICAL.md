...
`npm run performance:profile` reconstruye el build y captura una traza de arranque móvil con muestreo de CPU; `-- --desktop` selecciona escritorio. Escribe `reports/startup-profile.json`, ignorado por Git y fuera del deploy. Sus tiempos incluyen sobrecarga de instrumentación y no sustituyen el snapshot de 60 segundos. La prueba E2E guarda capturas de escritorio/móvil en `tmp`, eliminadas por la limpieza local.
